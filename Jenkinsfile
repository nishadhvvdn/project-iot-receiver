...
pipeline{
   ...
   parameters {
       choice(name: 'ENVIRONMENT_NAME', choices: ['DEV', 'DEV-TEST', 'DEV-TEST-NEW', 'QA', 'QA-LOAD-TEST', 'PROD', 'KCEC'], description: 'Select the environment to be deployed')
   }
    ...
    post{
        success{
            script{
		    badge("${BUILD_USER}")
		    build job: 'project-iot-receiver_CD',
